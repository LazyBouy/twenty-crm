...
- **`sed -i`, `awk -i inplace`, `perl -i`, `cat > <file>`, `cat >> <file>`, `echo > <file>`, `echo >> <file>`, `printf > <file>`, `printf >> <file>`, `tee <file>`, `tee -a <file>`, `cp <src> <repo-file>`, `mv <repo-file>`**. Any shell redirection, append, or copy that writes to a repo file path. The append variants (`>>`, `tee -a`) are forbidden even for "just one line, I'll revert it" — see the framing-trap warning below.
...
**The framing trap — "I'll just revert it after"**: the most common path into a violation is the inquisitive framing — *"let me append one bad line to test whether the lint gate catches it, then I'll restore it"*. This IS the violation. Two reasons it's never acceptable:

1. **The rationalization itself is the violation.** The rule isn't "don't make permanent changes" — it's "don't mutate working-tree files, full stop." A revert after the fact does not retroactively unmake the original mutation. The audit log shows: write happened, restore happened. That's two violations of a one-action rule, not zero.
2. **You can always inspect the gate's behaviour without mutating.** To verify what a lint/format gate does on bad input: read the gate's command text (the script in `project.json` or `.husky/`), reason about what `grep -E '\.(ts|tsx)$'` includes/excludes, etc. The gate's source code is the authoritative spec — running the gate against a deliberate violation tells you nothing the gate's source doesn't already say. If you can't reason about the gate without running it on tampered input, that's a sign the gate's logic is too opaque for you to audit confidently — write that observation in your report instead.

When in doubt: report the limitation. *"I cannot confirm gate X's behaviour on edge case Y without mutating working-tree files, which is forbidden — supervisor should run the test manually if confidence is needed"* is always preferred over *"I checked and it works (after reverting my tampering)"*.

### Why this section exists (L13 — institutional memory)

**Incident 1 — issue #14 audit-round-1 (2026-05-12)**: an auditor ran `prettier --write packages/twenty-mcp/src/tools/crm.ts` to inspect the formatting diff, then ran `git checkout -- packages/twenty-mcp/src/tools/crm.ts` to revert. Because the supervisor had in-flight unstaged work on `crm.ts` (the #12+#13 `resolveObjectNames` refactor awaiting commit), the `git checkout` silently discarded the entire refactor. The supervisor recovered the lost code by extracting the original TypeScript from the jest transform cache's sourcemap (`sourcesContent[0]`), but it cost ~15 minutes and produced a CRITICAL audit finding for a procedural error. **L13: auditor must never run destructive shell commands; use `--check` and read-only inspection only.**

**Incident 2 — clubbed #16/#17/#18 audit-round-1 (2026-05-12)**: an auditor doing adversarial testing of the newly-introduced `lint:diff-with-main` gate appended two intentionally-malformed lines to `packages/twenty-mcp/src/utils/view-filter-row.schema.ts` via `bash echo >>`, then restored the file via the `Write` tool. The audited diff happened to be restorable verbatim from an earlier `Read`, so no data was lost — but the violation was logged and self-disclosed. The framing was *"just a quick check that the gate catches prettier violations"*; the L13 rule was technically in the prohibition list at the time, but the auditor rationalized the append as an exception because they planned to revert it. **L_audit_procedural: the "I'll just revert it" framing IS the violation — see "The framing trap" warning above.** The general prohibition (line 223) now lists every common append form (`>>`, `tee -a`, `printf >>`, `cat >>`) explicitly, and the trap is named so future auditors recognize the pattern before falling into it.
